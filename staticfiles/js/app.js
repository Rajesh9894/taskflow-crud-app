...
function showTasks(tasks) {
  ...
  tasks.forEach((task) => {
    ...
    let description = task.description ? `<div class="task-desc">${task.description}</div>` : "";
    ...
    rows += `
      <tr>
        ...
        <td>
          <span class="badge badge-${task.status}">${statusText}</span>
        </td>
        <td>
          <span class="badge badge-${task.priority}">${priorityText}</span>
        </td>
        ...
        <td>
          <div class="actions">
            ...
            <button class="btn btn-danger btn-sm" onclick="openDeleteModal(${task.id}, '${task.title}')">Delete</button>
          </div>
        </td>
      </tr>`;
  });
...
async function loadStats() {
  try {
    ...
    document.getElementById("pageSub").textContent = data.total + " Tasks | " + data.done + " Completed";
  } catch (error) {
...
function debounceSearch() {
  ...
  searchTimer = setTimeout(function () { loadTasks(); }, 300);
}
...
function setView(status) {
  ...
  if (status === "all") document.getElementById("pageTitle").innerText = "All Tasks";
  else if (status === "todo") document.getElementById("pageTitle").innerText = "To Do Tasks";
  else if (status === "in_progress") document.getElementById("pageTitle").innerText = "In Progress Tasks";
  else if (status === "done") document.getElementById("pageTitle").innerText = "Done Tasks";
